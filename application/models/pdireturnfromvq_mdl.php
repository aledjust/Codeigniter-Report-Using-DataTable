...
class Pdireturnfromvq_mdl extends CI_Model{
    #code
    function __construct(){
        parent:: __construct();		
    }
		
	function table_pdireturnfromvq()
	{
		$aColumns = array('Model','FrmNo','Kode','Keterangan','Colour','Problem','JenisKerusakan','Image','Remark','NrkVq','NrkDelivery','InspectionDate');
		
		$sIndexColumn = "RowId";
		$sImageColumn = "Image";
		$primary = "RowId";
		
		$sTable = "txpdireturnfromvq";
		...
		foreach($rResult->result_array() AS $aRow){
			$row = array();
			 ...
			 for($i = 0 ; $i < count($aColumns); $i++){
				 ...
				 else if($aColumns[$i] == $sImageColumn) {
					 /* image output */
					 $url = $this->config->base_url()."asset/img/".$aRow[$aColumns[$i]];
					 $row[] = "<img class='contentImg' src='". $url ."'>";					 
				 }
				 else if($aColumns[$i] != ' ') {
					 /* general output */
					 $row[] = $aRow[$aColumns[$i]];					 
